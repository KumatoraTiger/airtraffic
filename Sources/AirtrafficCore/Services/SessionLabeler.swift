    ...
    public func label(
        ...
        let system = """
            ...
            - subject は対象が特定できる最短の日本語名詞句（15字程度）。
              PR 番号・リポジトリ名が読み取れるなら含めるが、識別子だけの subject は禁止。
              発言や todo から作業対象の内容（何の機能・何の変更か）を読み取り、必ず含める。
              例:「PR #123 のレビュー」は不可、「PR #123 認証リトライ修正」は可。
              内容が読み取れないセッションは出力に含めない。依頼文の写しにしない。
